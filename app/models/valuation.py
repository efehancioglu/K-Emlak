...
class Valuation(Base):
    ...
    # Tahmin sonucu.
    tahmini_fiyat: Mapped[int] = mapped_column(Integer)
    birim_m2_fiyat: Mapped[int] = mapped_column(Integer)
    fiyat_alt: Mapped[int] = mapped_column(Integer)
    fiyat_ust: Mapped[int] = mapped_column(Integer)

    # Kullanici bir beklenen fiyat girdiyse: fiyat + yorum ("uygun"/"normal"/
    # "pahali"). Girmediyse null kalir.
    beklenen_fiyat: Mapped[int | None] = mapped_column(Integer, nullable=True)
